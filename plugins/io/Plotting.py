...
class PlottingWaveform(plugin.OutputPlugin):
    def write_event(self, event):
        ...
        for peak in event.S2s() + event.S1s():
            ...
            ax.annotate('%s:%s' % (peak.peak_dict['peak_type'], int(peak.area())),
                        xy=(x, y),
        ...
        plt.plot(event.filtered_waveform('filtered_for_large_s2'), label='large_s2')
        plt.plot(event.filtered_waveform('filtered_for_small_s2'), label='small_s2')
