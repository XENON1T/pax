...
class PruneNonIsolatedPeaks(PeakPruner):
	# mean of test_before samples before interval must be less than before_to_height_ratio_max times the maximum value
	#  in the interval
	# Same for test_after
	...
	def decide_peak(self, peak, event, peak_index):
		...
		if peak['before_mean'] > settings['before_to_height_ratio_max'] * peak['height'] \
			...
			# return '%s samples before peak contain stuff (mean %s, which is more than %s (%s x peak height))' % (
			# settings['test_before'], peak['before_mean'], settings['before_to_height_ratio_max'] * peak['height'],
			# settings['before_to_height_ratio_max'])
...
class PruneS1sInS2Tails(PeakPruner):
	def decide_peak(self, peak, event, peak_index):
		...
		if not 'stop_looking_for_s1s_after' in event:
			...
			larges2boundaries = [p['left'] for p in event['peaks'] if
			                     is_s2(p) and p['top_and_bottom']['height'] > treshold]
		...
		if peak['left'] > event['stop_looking_for_s1s_after']:
			return 'S1 starts at %s, which is beyond %s, the starting position of a "large" S2.' % (
			peak['left'], event['stop_looking_for_s1s_after'])
		return None
...
class PruneS2sInS2Tails(PeakPruner):
	def decide_peak(self, peak, event, peak_index):
		...
		if peak['left'] > event['stop_looking_for_s2s_after']:
			return 'S2 starts at %s, which is beyond %s, the starting position of a "large" S2.' % (
			peak['left'], event['stop_looking_for_s2s_after'])
		return None
