...
class TestBSON(unittest.TestCase):
    ...
    def test_write_event(self):
        self.addCleanup(rmtree, 'bsontest_tempdir')

        config = {'pax': {
            'plugin_group_names': ['output'],
            'output': 'BSON.WriteBSON',
        },
            'BSON': {'output_name': 'bsontest_tempdir'}
        }
        ...
        write_plugin = proc.get_plugin_by_name('WriteBSON')
    ...
    def test_write_read(self):
        self.addCleanup(rmtree, 'bsontest_tempdir2')
        config = {'pax': {
            'events_to_process': [0],
            ...
            'BSON': {
            'output_name': 'bsontest_tempdir2',
            'input_name': 'bsontest_tempdir2'
        }
        }
