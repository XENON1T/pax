...
class MeanShift(plugin.TransformPlugin):
    ...
    def transform_event(self, event):
        ...
        for detector in self.config['channels_in_detector'].keys():
            self.log.debug("Clustering channel peaks in data "
                           "from %s" % detector)
                ...
                for ci in cluster_indices:
                    ...
                    for label in np.unique(ms.labels_):
                        ...
                        peaks_in_cluster = np.unique(data[1,
                                                          (ms.labels_ == label)])
                        new_cluster_indices.append(channel_peak_objects[peaks_in_cluster])

                # Each cluster becomes a peak
                # Compute basic properties, check for too many lone pulses per channel
                for ci in new_cluster_indices:
                    peak = datastructure.Peak({
                        'channel_peaks':            ci.tolist(),
                        'detector':                 detector,
                        'area_per_channel':         np.zeros(self.n_channels),
                        'does_channel_contribute':  np.zeros(self.n_channels,
                                                             dtype='bool'),
                        'does_channel_have_noise':  np.zeros(self.n_channels,
                                                             dtype='bool'),
                    })
                    ...
                    if peak.number_of_contributing_channels == 0:
                        raise RuntimeError("Every peak should have at least "
                                           "one contributing channel... what's "
                                           "going on?")
...
class HitGap(plugin.TransformPlugin):
    """Clusters hits based on times between them
    ...
    def transform_event(self, event):
        ...
        for detector in self.config['channels_in_detector'].keys():
            ...
            while redo_classification:
                ...
                # Cluster the single-pes in groups separated by >= self.s2_width
                cluster_indices = utils.cluster_by_diff([s.index_of_maximum * self.dt for s in spes],
                                                        self.s2_width, return_indices=True)
                self.log.debug("Made %s clusters" % len(cluster_indices))
