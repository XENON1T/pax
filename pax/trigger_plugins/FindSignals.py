...
class FindSignals(TriggerPlugin):
    ...
    def startup(self):
        ...
        # Initialize the coincidence tally matrix
        # Reason for +1 is again 'ghost' channels, see trigger.py
        n_channels = len(self.pmt_data)
        self.coincidence_tally = np.zeros((n_channels + 1, n_channels + 1), dtype=np.int)

        # Create the dark rate monitoring datasets
        f = self.trigger.dark_monitor_data_file
        self.dark_rate_dataset = f.create_dataset('dark_pulses',
