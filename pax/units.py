"""
This sets up variables for the various unit abbreviations, ensuring we always have a 'consistent' unit system.
You can change base_units without breaking the consistency of any physical relations

Please change comments in this code if you change any units. Better yet, don't change any units!

WARNING
    The unit system is not fully consistent, since the unit for energy is set separately from mass, time, and distance
...
base_units['A'] = base_units['C']/base_units['s']   #Current will be in electrons/ns!
...
for (name,value) in list(base_units.items()):
    for (p_name,p_factor) in list(prefixes.items()):
        vars()[p_name+name] = float(10**(p_factor) * value)   #Float makes sure units might work even for poor fellas who forget to from __future__ import division -- not tested though
