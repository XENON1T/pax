...
probability of an isotropically produced photon to arrive (and be detected?) by the PMT.
Also contains a map named 'total_LCE' describing the probability of an isotropically 
produced photon to arrive (and be detected?) by ANY PMT in the top array.
...
def convert_lce_map(filename, outname, n_pmts=98):
    ...
    # Jelle 18 Nov: I manually changed y_min to -235 and n_y to 95 in the map
    ...
    output = {pmt+1 : np.zeros((n_x, n_y)).tolist() for pmt in range(n_pmts)}
    ...
    for line_index, line in enumerate(tqdm(raw_map[4:])):
        ...
        for pmt, fraction_of_light_received in enumerate(linedata[3:]):
            ...
            output[pmt][x_index][y_index] = result
